...
void main() {
  group('LudoCaptureBurstComponent', () {
    testWithFlameGame(
      'a full-motion burst removes itself once its bounded lifetime ends',
      (game) async {
        final burst = LudoCaptureBurstComponent(position: Vector2.all(50));
        ...
        while (game.children.contains(burst)) {
          ...
          elapsed += dt;
          if (elapsed > 3) {
            fail('capture burst never self-removed (unbounded lifetime)');
          }
        }
        ...
        expect(
          elapsed,
          lessThan(1.0),
          reason: 'capture burst must have a short, bounded lifetime',
        );
      },
    );
    ...
    testWithFlameGame(
      ...
      (game) async {
        ...
        // The reduced-motion flash's lifespan is far shorter than a
        // full-motion burst's.
        expect(
          burst.progress,
          lessThan(1.0),
          reason: 'flash should not have already finished on the first frame',
        );
