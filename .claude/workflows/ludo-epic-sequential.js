...
- Never boot emulators/simulators. Physical-device or Firebase-console steps are reported as NOT RUN, never faked.
...
4. Commit on ${BRANCH} with the exact commit message given in the task file (format <type>(<scope>): <summary> [15-ludo-launch/<NN>]). Let pre-commit hooks run; if a hook fails, fix the cause within this task's files and retry once; never use --no-verify.
