export const meta = {
  name: 'game-epic-sequential',
  description: 'Execute a game launch epic one task at a time with a single sonnet agent per step: implement, independently verify, fix (max 2), commit',
  whenToUse: 'Running a tasks/epics game epic strictly in order with verification and device gates',
  phases: [
...
// args: { repo: '/abs/repo', game: 'Ludo', appId: 'ludo', branch: 'main', epicDir: 'tasks/epics/NN-slug', epicTag: 'NN-slug',
//         deviceSerial: 'RZ8R32EAB7T', deviceDesc: 'Samsung A52, 1080x2400', frozenDirs: ['apps-native/unity/ludo'],
//         visualTarget: '<one paragraph: reference README + anchor screenshots>', evidenceRoot: '.agents/resources/<date>/<game>-visual-qa',
//         commitTrailer: 'Claude-Session: <url>' (optional), resumeDirtyPaths: [...], tasks: [...], stopAfter: 'NN' }
// (original Ludo run used: branch 'main', epicDir 'tasks/epics/15-ludo-launch',
...
const EPIC = A.epicDir
const EPIC_TAG = A.epicTag || EPIC.split('/').pop()
const REPO = A.repo || '.'
const GAME = A.game || 'game'
const APP_ID = A.appId || ''
const SERIAL = A.deviceSerial || ''
const DEVICE_DESC = A.deviceDesc || ''
const FROZEN = Array.isArray(A.frozenDirs) ? A.frozenDirs : []
const VISUAL = A.visualTarget || ''
const EVIDENCE = A.evidenceRoot || '.agents/resources/<date>/' + GAME.toLowerCase() + '-visual-qa'
...
- Do only what the task file scopes.${FROZEN.length ? ' Do not modify frozen dirs: ' + FROZEN.join(', ') + ' (except a doc note if the task says so).' : ''}
- Never boot emulators/simulators. ${SERIAL ? `A physical Android device (adb serial ${SERIAL}${DEVICE_DESC ? ', ' + DEVICE_DESC : ''}) is attached: when a task's Verification Commands include device steps, actually run them (build, adb install, launch, navigate with adb input/uiautomator, screencap) and VIEW every screenshot with the Read tool. Only if adb shows the device missing, report NOT RUN.` : 'No device is attached: device steps are reported as NOT RUN, never faked.'} Console/provisioning steps (Firebase, stores) are reported as NOT RUN, never faked.
${VISUAL ? '- Visual target: ' + VISUAL + ' Judge on-device screenshots against it critically.' : ''}
- Artifacts: save EVERY screenshot or visual evidence file you produce (device captures, comparisons) under ${EVIDENCE}/<task-id>/ with descriptive names, overriding any other evidence path a task file names, and include them in the task's commit. Never leave evidence only in /tmp.
...
- Flutter tests: a healthy app test suite finishes in well under 3 minutes. While iterating, run single files with a per-test timeout (e.g. \`flutter test --timeout 60s test/path_test.dart\` from apps-native/games/${APP_ID}). A slow or hanging test is a BUG in this task's code or tests (typically pumpAndSettle on a repeating/infinite animation — use pump(duration) or disable the looping animation in tests, or unresolved timers), never a reason to return blocked. Always read the actual pass/fail output before claiming a result.
