...
- [x] Run the image-tool smoke test and log the tool, model and parameters.
- [x] Generate the preview sets and VIEW every image; regenerate once if an
      image is off-brief.
- [x] Create the mockups (`mockup-A.png`, `-B`, `-C`) and a `contact-sheet.png`.
- [x] Write the `README.md` (prompts per file, the tool and settings, a
      direction summary, pros and cons, the cost sheet, and a recommendation).
- [x] Add an open question for the user's pick to
      `.agents/games/pocket-biome/open-questions.md`.

Completion note (2026-09-25): all 33 images were generated by the first agent,
which then hit the usage limit. A second agent built the mockups, contact
sheet and README from those images without regenerating any. The exact
per-image prompts were lost; the README gives reconstructed direction briefs
labelled as such (a deviation from "logs the prompt for every file").
